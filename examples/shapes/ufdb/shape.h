...
class DBShape : public DBDomainObj
{
  UF_DB_DOMAIN_PROXY_DEF(DBShape)
public:
  DBShape();
  virtual void init(int itemId, int itemType, double posX, double posY, int r, int g, int b);
  // DBShape(const DBShape & copy);
  // DBShape & do_copy(bool new_ids=false, HASH id_scope=NULL, // 				  HASH id_remap=NULL);
