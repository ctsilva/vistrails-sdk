...
DBDomainObj * DBShapeXMLDAOBase::fromXML(TiXmlElement * elt)
{
  assert(elt);
  ...
  DBShape * obj = DBDomainFactory::getGlobalFactory()->createShape();
  obj->init(itemId, itemType, posX, posY, r, g, b);
  obj->is_dirty = false;
  return obj;
}

TiXmlElement * DBShapeXMLDAOBase::toXML(const DBDomainObj * vt_obj,
					      TiXmlElement * elt)
{
  const DBShape * obj = dynamic_cast<const DBShape *>(vt_obj);
  if (elt == NULL) {
    elt = new TiXmlElement("shape");
  }
