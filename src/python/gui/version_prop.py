...
from common_widgets import QSearchBox
# FIXME using python's builtin (since 2.5 so should be ok)
# from core.utils import all
# FIXME debug only used for search which is disabled
...
class QVersionProp(QtGui.QWidget, QToolWindowInterface):
    ...
    def __init__(self, parent=None):
        ...
        self.tagReset.setIcon(self.style().standardIcon(
                QtGui.QStyle.SP_TitleBarCloseButton))
        self.tagReset.setStyleSheet("QToolButton { border: none; "
                                    "padding: 0px; }")
    ...
    def updateVersion(self, version):
        if version is not None:
            self.versionNotes.setText("")
            self.versionNotes.setEnabled(True)
            self.tagEdit.setText("")
            self.tagEdit.setEnabled(True)
            date = version.getDate()
            utc_dt = datetime.strptime(date, "%Y-%b-%d %H:%M:%S")
            utc_dt = utc_dt.replace(tzinfo=dateutil.tz.gettz('UTC'))
            local_dt = utc_dt.astimezone(dateutil.tz.tzlocal())
            if local_dt.tzname() is not None:
                local_dt_str = local_dt.strftime("%Y-%b-%d %H:%M:%S %Z")
            else:
                local_dt_str = local_dt.strftime("%Y-%b-%d %H:%M:%S %z")
            self.dateEdit.setText(local_dt_str)
            self.userEdit.setText(version.getUser())
            self.idEdit.setText(version.getId()[0:13] + "...")
        else:
            self.versionNotes.setText("")
            self.versionNotes.setEnabled(False)
            self.tagEdit.setText("")
