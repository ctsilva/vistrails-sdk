...
# FIXME disabling thumbnails
# from core.thumbnails import ThumbnailCache
# from vistrails_tree_layout_lw import VistrailsTreeLayoutLW
from tree_layout_lw import TreeLW, TreeLayoutLW, NodeLW
from graphics_view import (QInteractiveGraphicsScene,
                           QInteractiveGraphicsView,
                           QGraphicsItemInterface,
                           QGraphicsRubberBandItem)
from qt import qt_super
from theme import CurrentTheme
from version_prop import QVersionPropOverlay
# FIXME disabling diff support for now
# from gui.vis_diff import QVisualDiff
# FIXME disabling show_question in pruning
# import gui.utils

class LayoutNode(NodeLW):
    def __init__(self, x=0, y=0, width=0.0, height=0.0, node_id="", tag=""):
        NodeLW.__init__(self, width, height, (node_id, tag))
        self.x = x
        self.y = y
...
class LayoutTree(TreeLW):
    def addExistingNode(self, newNode, parentNode=None):
...
class QGraphicsVersionItem(QGraphicsItemInterface, QtGui.QGraphicsEllipseItem):
    ...
    def setupVersion(self, node, action, tag, description):
        ...
        rect = QtCore.QRectF(node.x-node.width/2.0,
        ...
        # else:
        #     self.setToolTip('')    
        self.text.changed(node.x, node.y, textToDraw, validLabel)
        self.setRect(rect)
    ...
    def show_raw_pipeline(self):
        ...
        self.scene().emit(QtCore.SIGNAL('versionSelected(QString *,bool,bool,bool)'),
...
#             max_rank = ourMaxRank if nodeUser==currentUser else otherMaxRank
    ...
    def layout_tree(self, tree):
        print "starting layout", id(tree)
        child_id = tree.getFirstChild("00000000-0000-0000-0000-000000000000")
        print "child_id:", child_id
        nodes = [("00000000-0000-0000-0000-000000000000", "ROOT")]
        edges = []
        ...
        while len(version_ids) > 0:
            ...
            if version != None:
                print version.getId(), version.getParent(), version.getUser()
                nodes.append((version.getId(), version.getId()[:8] + "..."))
                edges.append((version.getParent(), version.getId()))
            print 'trying get child', version_id
        ...
        for node_id, tag in nodes:
            ...
            node = LayoutNode(0.0, 0.0, width, height, node_id, tag)
            tree_lw.addExistingNode(node)
            mapTreeNodes[node_id] = node
        ...
        for (parentId, childId) in edges:
            ...
            #     raise Exception()
            tree_lw.changeParentOfNodeWithNoParent(parent, child)


        min_horizontal_separation = 20
        min_vertical_separation = 50

        layout = TreeLayoutLW(tree_lw, TreeLayoutLW.TOP,
                              min_horizontal_separation,
                              min_vertical_separation)

        # prepare the result
        # layout_nodes = {}
        # for node in tree_lw.nodes:
        #     node_id, tag = node.object
        #     newNode = LayoutNode(node.x, node.y, node.height, node.width, 
        #                          node_id, tag)
        #     layout_nodes[node_id] = newNode
    ...
    def setupScene(self, tree, selected_version=None):
        ...
        layout_nodes = self.layout_tree(tree)

        for node in layout_nodes.itervalues():
            shape = self.addVersion(node, None, node.tag, None)
            if node.id == selected_version:
                shape.setSelected(True)

        for node in layout_nodes.itervalues():
            for child in node.children:
                self.addLink(self.versions[node.id], self.versions[child.id], 
                             False, False)
    ...
    def selectionChanged(self):
        print 'calling selectionChanged!', self.emit_selection
        ...
        if len(selected_items) == 1:
            ...
            print "emiting versionSelected 1"
            self.emit(QtCore.SIGNAL('versionSelected(QString *,bool,bool,bool)'),
            ...
            print "emiting versionSelected ''"
            self.emit(QtCore.SIGNAL('versionSelected(QString *,bool,bool,bool)'),
                      "", self.select_by_click, True, False)

        if len(selected_items) == 2:
            print "emiting versionSelected 2"
            self.emit(
                QtCore.SIGNAL('twoVersionsSelected(QString *, QString *)'),
...
class QVersionTreeView(QInteractiveGraphicsView):
    ...
    def setupScene(self, tree, selected_version=None):
        self.scene().setupScene(tree, selected_version)
